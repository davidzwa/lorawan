    ...
    namespace lorawan
    {
        ...
        EndDeviceLorawanMac::DoSend(Ptr<Packet> packet)
        {
            ...
            if (packet != m_retxParams.packet)
            {
                ...
                // TODO add device class base method
                ...
                if (packet->GetSize() > m_maxAppPayloadForDataRate.at(m_dataRate))
                {
                    NS_LOG_WARN("Attempting to send a packet larger than the maximum allowed"
                                << " size at this DataRate (DR" << unsigned(m_dataRate) << "). Transmission canceled.");
                    return;
                }
                ...
                if (m_retxParams.waitingAck)
                {
                    ...
                    NS_LOG_DEBUG(" Received new packet from the application layer: stopping retransmission procedure. Used " << unsigned(txs) << " transmissions out of a maximum of " << unsigned(m_maxNumbTx) << ".");
                }
        ...
        EndDeviceLorawanMac::GetNextTransmissionDelay(void)
        {
            ...
            for (it = logicalChannels.begin(); it != logicalChannels.end(); ++it)
            {
                ...
                NS_LOG_DEBUG("Waiting time before the next transmission in channel with frequecy " << frequency << " is = " << waitingTime.GetSeconds() << ".");
            }
                                          ...
                                          std::list<int> enabledChannels, int repetitions)
        {
            ...
            if (dataRateOk && channelMaskOk) // If false, skip the check
            {
                ...
                for (auto it = enabledChannels.begin(); it != enabledChannels.end(); it++)
                {
                    ...
                    if (channelList.at(*it)->GetMinimumDataRate() <= dataRate && channelList.at(*it)->GetMaximumDataRate() >= dataRate)
                    {
                                               ...
                                               uint8_t minDataRate, uint8_t maxDataRate)
        {
            ...
            m_channelHelper.SetChannel(chIndex, CreateObject<LogicalLoraChannel>(frequency, minDataRate, maxDataRate));
        }
